...
(async () => {
  ...
  for (const v of VIEWS) {
    const r = await page.evaluate(async (v) => { window.__calls = {}; const s = performance.now(); document.querySelector('.tab[data-view="' + v + '"]').click(); await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r))); const ms = Math.round(performance.now() - s); return { active: document.querySelector('.view.active') && document.querySelector('.view.active').id, tabActive: (document.querySelector('.tab.active') || {}).dataset && document.querySelector('.tab.active').dataset.view, calls: { ...window.__calls }, ms, overflow: document.documentElement.scrollWidth > document.documentElement.clientWidth }; }, v);
    const fn = RENDER_OF[v]; const ok = r.active === 'view-' + v && r.tabActive === v && (fn ? r.calls[fn] >= 1 : true);
